...
protocol MJSwipeCardsDelegate {
    func swipeDidEnd(on view: MJSwipeCardView)
}

class MJSwipeCardView: UIView {
    ...
    var delegate: MJSwipeCardsDelegate?
    ...
    @objc func handlePanGesture(sender: UIPanGestureRecognizer){
        let card = sender.view as! MJSwipeCardView
        ...
        case .ended:
            if card.center.x > 350 {
                ...
                delegate?.swipeDidEnd(on: card)
                UIView.animate(withDuration: 0.2) {
                  card.center = CGPoint(x: centerOfParentContainer.x + point.x, y: centerOfParentContainer.y + point.y - 100)
                  card.alpha = 0
                  self.layoutIfNeeded()
                }
                return
